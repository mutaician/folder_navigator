...
def get_number_of_lines():
    while True:
        lines = input(f"Enter amount of lines to bet on (1-{MAX_LINES})? ")
        if lines.isdigit():
            lines = int(lines)
...
def get_bet():
    while True:
        bet = input(f"What would you like to bet (min: {MIN_BET}, max: {MAX_BET})? $")
        if bet.isdigit():
            bet = int(bet)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(
                f"You don't have enough money to bet that amount, your current balance is {balance}."
            )
        else:
            break
